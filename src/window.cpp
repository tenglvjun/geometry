...
void GeoWindow::OnMouseMove(double xpos, double ypos)
{
    ...
    if (m_mouseRBtnDown)
    {
        GeoVector4D v2 = GeoRender::GetInstance()->MapScreenToModel(GeoVector4D(now, 1.0f), 0);
        GeoVector4D v1 = GeoRender::GetInstance()->MapScreenToModel(GeoVector4D(lastPt, 1.0f), 0);
    ...
    if (m_mouseLBtnDown)
    {
        ...
        GeoVector4D cs1 = GeoRender::GetInstance()->MapModelToScreen(center, 0);
        mesh->Transform(rotate);
        GeoVector4D cs2 = GeoRender::GetInstance()->MapModelToScreen(center, 0);
        GeoVector4D ts = cs1 - cs2;
        GeoVector4D tm = GeoRender::GetInstance()->MapScreenToModel(ts, 0);
